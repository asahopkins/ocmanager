...
ARGV.clone.options do |opts|
  opts.on("-p", "--port=port", Integer, "Runs Rails on the specified port.", "Default: 3010") { |v| OPTIONS[:port] = v }
  opts.on("-b", "--binding=ip", String, "Binds Rails to the specified ip.", "Default: 0.0.0.0") { |v| OPTIONS[:ip] = v }
  opts.on("-d", "--daemon", "Make server run as a Daemon.") { OPTIONS[:detach] = true }
  opts.on("", "--prefix=PATH", String, "Mount rails at a subdirectory.", "Default: /manager") { |v| OPTIONS[:prefix] = v }
...
if OPTIONS[:prefix]
  parameters = parameters + ["--prefix", OPTIONS[:prefix]]
end
