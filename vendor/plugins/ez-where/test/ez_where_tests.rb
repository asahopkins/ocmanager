...
class EZWhereTest < Test::Unit::TestCase
   ...
   def test_composition_in_steps
      ...
      assert_equal expected, cond.to_sql
      cond += c(:other_table) { gem === (1..5) }
      expected = ["(((my_table.name = ?) OR (my_table.name LIKE ?))) AND (other_table.gem IN (?))", "fab", "ez%", [1, 2, 3, 4, 5]]
      assert_equal expected, cond.to_sql
      cond -= 'age > 20'
      expected = ["((((my_table.name = ?) OR (my_table.name LIKE ?))) AND (other_table.gem IN (?))) AND NOT age > 20", "fab", "ez%", [1, 2, 3, 4, 5]]
      assert_equal expected, cond.to_sql     
      cond |= c(:other_table) { login_count <=> (1..5) }
      expected = ["(((((my_table.name = ?) OR (my_table.name LIKE ?))) AND (other_table.gem IN (?))) AND NOT age > 20) OR (other_table.login_count BETWEEN ? AND ?)", "fab", "ez%", [1, 2, 3, 4, 5], 1, 5]
      assert_equal expected, cond.to_sql
   end
   ...
   def test_cc_with_multiple_statements
      ...
      c = c { age > 20 }
      d = c { gem === (1..5) }
      expected = ["((foo = ?) AND (baz LIKE ?)) OR ((age > ?) AND NOT (gem IN (?)))", "bar", "%qux%", 20, [1, 2, 3, 4, 5]]
      assert_equal expected, (a + b | c - d).to_sql
   end

   def test_create_condition_and_respect_table_name
      cond = c(:my_table) { combine c { baz =~ '%qux%' } + c { gem === (1..5) } | c(:other_table) { foo == 'bar' } }
      expected = ["(((my_table.baz LIKE ?) AND (my_table.gem IN (?))) OR (other_table.foo = ?))", "%qux%", [1, 2, 3, 4, 5], "bar"]
      assert_equal expected, cond.to_sql
   ...
   def test_cc_with_statements_inline_contrived_example
     ...
     expected = ["((my_table.foo = ? AND my_table.baz LIKE ? AND (my_table.name = ? OR my_table.name = ?)) AND age > 20) AND NOT (number = ? AND color = ?)", "bar", "%buzz%", "fab", "ezra", "five", "red"]
     assert_equal expected, cond.to_sql
   end
   ...
   def test_nested_include_conditions
     # any articles written by or commented on by Fab, using subcondition
     articles = Article.find_where(:all, :include => [:author, { :comments => :users }]) do |article, author, comment|
        article.author_id == 2
        author.name == 'Ezra'
        comment.body =~ '%Lorem%'
     end
   ...
   def test_within_time_delta
     ...
     expected = "[\"(articles.created_at BETWEEN ? AND ?)\", Sun Jan 01 00:00:00 CET 2006, Mon Jan 01 00:00:00 CET 2007]"
     assert_equal expected, cond.to_sql.inspect
     cond = Article.c { created_at.within_time_delta(2006, 6) }
     expected = "[\"(articles.created_at BETWEEN ? AND ?)\", Thu Jun 01 00:00:00 CEST 2006, Sat Jul 01 00:00:00 CEST 2006]"
     assert_equal expected, cond.to_sql.inspect
     cond = Article.c { created_at.within_time_delta(2006, 6, 15) }
     expected = "[\"(articles.created_at BETWEEN ? AND ?)\", Thu Jun 15 00:00:00 CEST 2006, Fri Jun 16 00:00:00 CEST 2006]"
     assert_equal expected, cond.to_sql.inspect
     cond = Article.c { created_at.within_time_delta(Time.local(2006, 6, 15)) }
     assert_equal expected, cond.to_sql.inspect
   end
   
   def test_day_range
     cond = Article.c
     cond.created_at.days_ago(7, Time.local(2006, 6, 15))
     expected = "[\"(articles.created_at BETWEEN ? AND ?)\", Thu Jun 08 00:00:00 CEST 2006, Thu Jun 15 00:00:00 CEST 2006]"
     assert_equal expected, cond.to_sql.inspect
     cond = Article.c { created_at.days_ago(3..5, Time.local(2006, 6, 15)) }
     expected = "[\"(articles.created_at BETWEEN ? AND ?)\", Sat Jun 10 00:00:00 CEST 2006, Mon Jun 12 00:00:00 CEST 2006]"
     assert_equal expected, cond.to_sql.inspect
     cond = Article.c { created_at.days_ago(30, Time.local(2006, 7, 1)) }
     expected = "[\"(articles.created_at BETWEEN ? AND ?)\", Thu Jun 01 00:00:00 CEST 2006, Sat Jul 01 00:00:00 CEST 2006]"
     assert_equal expected, cond.to_sql.inspect
     
     cond = Article.c(:inner => :or) { created_at.days_ago(7, Time.local(2006, 6, 15)); updated_at.days_ago(7, Time.local(2006, 6, 15)) } + Article.c { user_id == 2 }
     expected = "[\"((articles.created_at BETWEEN ? AND ?) OR (articles.updated_at BETWEEN ? AND ?)) AND (articles.user_id = ?)\", Thu Jun 08 00:00:00 CEST 2006, Thu Jun 15 00:00:00 CEST 2006, Thu Jun 08 00:00:00 CEST 2006, Thu Jun 15 00:00:00 CEST 2006, 2]"
     assert_equal expected, cond.to_sql.inspect
   end
